...
if [ "$#" -ne 1 ]
then
  echo "Provide twitter username as the first parameter: ./login.sh myTwitterUserName"
  exit 1
fi
...
twitter=$(curl $urlToCall -H 'upgrade-insecure-requests: 1' -H 'cache-control: no-cache' -H 'authority: api.twitter.com' -H 'referer: http://'$host'/' --compressed -i 2>/dev/null)
...
oauthVerifier=$(sed -n -e 's/^.*\/signin\/twitter.*\&oauth_verifier\=//Ip' <<< "$authentication" | sed -n -e 's/\">$//p' | tr -d '\012\015')

logged=$(curl 'http://'$host'/signin/twitter?oauth_token='$oauth_token'&oauth_verifier='$oauthVerifier'' -H 'Pragma: no-cache' -H 'Accept-Encoding: gzip, deflate, sdch' -H 'Accept-Language: es,en-US;q=0.8,en;q=0.6' -H 'Upgrade-Insecure-Requests: 1' -H 'Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8' -H 'Cache-Control: no-cache' -H 'x-auth-token:'$xAuthToken'' -H 'Connection: keep-alive' --compressed -i 2>/dev/null)
